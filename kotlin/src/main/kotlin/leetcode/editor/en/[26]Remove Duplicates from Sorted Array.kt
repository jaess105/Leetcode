...
// 
//
// Related Topics Array Two Pointers 👍 9049 👎 12639

class RemoveDuplicatesFromSortedArray {
    //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        fun removeDuplicates(nums: IntArray): Int {
            var indexLastDistinct = 0

            for (i in 1 until nums.size) {
                if (nums[i] != nums[indexLastDistinct]) {
                    nums[++indexLastDistinct] = nums[i]
                }
            }
            return indexLastDistinct + 1
        }
    }
