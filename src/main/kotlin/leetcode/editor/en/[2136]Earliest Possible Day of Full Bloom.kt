...
// 
//
// Related Topics Array Greedy Sorting 👍 701 👎 39

class EarliestPossibleDayOfFullBloom {
    //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        fun earliestFullBloom(plantTime: IntArray, growTime: IntArray): Int {
            val map = mutableMapOf("plantTime" to 0, "growTime" to 0)
            for ((p, g) in plantTime.zip(growTime)
                .sortedByDescending { (_, g) -> g }) {
                map["plantTime"] = map["plantTime"]!! + p
                map["growTime"] = max(map["growTime"]!! - p, g)
            }
            return map["growTime"]!! + map["plantTime"]!!
        }
    }
    //leetcode submit region end(Prohibit modification and deletion)

    fun earliestFullBloom(plantTime: IntArray, growTime: IntArray): Int {
        return plantTime.zip(growTime)
            .sortedByDescending { (p, g) -> g }
            .fold(mutableMapOf("plantTime" to 0, "growTime" to 0))
            { coll, (p, g) ->
                coll["growTime"] = max(coll["growTime"]!! - p, g)
                coll["plantTime"] = coll["plantTime"]!! + p
                coll
            }.let { map -> map["growTime"]!! + map["plantTime"]!! }
    }
}