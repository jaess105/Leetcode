...
class WhereWillTheBallFall {
    ...
    class Solution {
        fun findBall(grid: Array<IntArray>): IntArray {
            val ret = IntArray(grid[0].size)
            for (ball in 0 until grid[0].size) {
                var currRow = 0
                var currCol = ball
                while (currRow != grid.size) {
                    // doesn't matter if left or right
                    val nextCol = currCol + grid[currRow][currCol]
                    if (// check not stuck on Wall
                        nextCol > (grid[0].size - 1) || nextCol < 0
                        // check not stuck
                        || grid[currRow][currCol] != grid[currRow][nextCol]
                    ) {// ball stuck
                        ret[ball] = -1
                        break
                    } else {// ball can fall down
                        currRow++ // row always increases when the ball falls down
                        currCol = nextCol
                    }
                }
                if (ret[ball] != -1) {
